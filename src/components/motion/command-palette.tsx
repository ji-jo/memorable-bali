...
export function CommandPalette({
  ...
  useEffect(() => {
    if (open) {
      // Reset the palette each time it opens so keyboard users always start
      // from the full place and category index.
      // eslint-disable-next-line react-hooks/set-state-in-effect
      updateQuery("");
      setActive(0);
    ...
    >
      ...
      <div className="pointer-events-none absolute inset-0 flex items-start justify-center p-4 pt-[18vh]">
        <motion.div
          ...
          onKeyDown={onKeyDown}
          className={cn(
            "w-full max-w-xl overflow-hidden rounded-2xl bg-[var(--color-surface)] will-change-transform",
            open ? "pointer-events-auto" : "pointer-events-none",
          )}
        >
          <div className="flex items-center gap-3 border-b border-[color:var(--color-border-strong)] px-5 py-3.5">
            ...
            <input
              ...
              className="h-11 flex-1 bg-transparent text-sm text-foreground placeholder:text-muted-foreground outline-none focus-visible:outline-none"
            />
            <kbd className="hidden shrink-0 rounded bg-[var(--color-surface-sunken)] px-1.5 py-0.5 text-[10px] text-muted-foreground sm:inline-block">
              ESC
            </kbd>
          </div>
          <div
            ...
            role="listbox"
            aria-label="Commands"
            className="max-h-[60vh] overflow-y-auto px-2 pb-2 pt-2"
          >
            {filtered.length === 0 ? (
                    ...
                    return (
                      ...
                      >
                        {isActive ? (
                          <motion.span
                            layoutId={`${uid}-active`}
                            className="absolute inset-0 z-0 rounded-md bg-[var(--color-surface-sunken)]"
                            transition={
                              reduce
                                ? { duration: 0 }
                        ...
                        {it.hint ? (
                          <kbd className="relative z-10 rounded bg-[var(--color-surface-sunken)] px-1.5 py-0.5 text-[10px] text-muted-foreground">
                            {it.hint}
                          </kbd>
                        ) : null}
                      </button>
